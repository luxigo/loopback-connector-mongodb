...
MongoDB.prototype.buildWhere = function(model, where, options) {
  ...
  Object.keys(where).forEach(function(k) {
    ...
    var propIsForeignKey = (prop === undefined);
    ...
    if (spec) {
      if (spec === 'between') {
        if (propIsForeignKey) {
          cond[0] = ObjectID(cond[0]);
          cond[1] = ObjectID(cond[1]);
        }
        ...
        query[k] = {
          $in: cond.map(function(x) {
            if (propIsForeignKey || self.isObjectIDProperty(model, prop, x, options))
              return ObjectID(x);
            return x;
          }),
        };
      ...
      if (cond === null) {
        ...
        if (propIsForeignKey || self.isObjectIDProperty(model, prop, cond)) {
          cond = ObjectID(cond);
        }
        query[k] = cond;
      }
